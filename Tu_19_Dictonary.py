# Dictonary in python

detailOne = {'name': 'Aman', 'age': '15 years', 'status': 'learning_Python'}
detailTwo = {100: 3, 200: 5, 300: 6}

#-------> Dictonary methods

# detailOne.update(detailTwo) ---> It merge the two dictonary and update the firstOne
# detailOne.clear() ---> It clear dictonary
# detailOne.pop('age') ---> It remove a particular given element from the dictonary
# detailOne.popitem()  ---> It remove the last element from the dictonary


print(detailOne) 
 